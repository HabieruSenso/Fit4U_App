...
import com.utad.Fit4U_GymApp_App.databinding.FragmentUserInfoBinding
...
class UserInfoFragment:Fragment(R.layout.fragment_user_info) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        eventosActualesUsuario()

    }

    override fun onStart() {
        super.onStart()
        userViewModel.obtenerUsuarioActual()
    }

    private fun eventosActualesUsuario() = lifecycleScope.launch{
        userViewModel.currentUserState.collect { result ->
            when(result){
                is Result.Success -> {
                    usarioLoggedIn()
                    binding?.userTxt?.text = result.data?.nombre ?: "NO Name"
                    binding?.userEmail?.text = result.data?.email ?: "No Email"
                }
                is Result.Error -> {
                    usarioNotLoggedIn()
                    binding?.userTxt?.text = "Únete"
                }
                is Result.Loading -> {
